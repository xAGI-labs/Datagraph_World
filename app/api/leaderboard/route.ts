...
async function fetchExternalBenchmarks() {
  try {
    const arenaResponse = await fetch('https://huggingface.co/spaces/lmsys/chatbot-arena-leaderboard/resolve/main/leaderboard_table_20241201.csv', {
      headers: {
...
export async function GET(request: NextRequest) {
  try {
      ...
      modelStats.map(async (stat) => {
        ...
        const avgResponseTime = totalResponses > 0 
          ? ((modelAResponses._avg?.responseTimeA || 0) * countA + 
             (modelBResponses._avg?.responseTimeB || 0) * countB) / totalResponses
          : 0;
    ...
    const modelLabels: { [key: string]: { label: string; provider: string } } = {
      ...
      'anthropic/claude-3.5-sonnet': { label: 'Claude 3.5 Sonnet', provider: 'Anthropic' },
      'anthropic/claude-3-haiku': { label: 'Claude 3 Haiku', provider: 'Anthropic' },
      'meta-llama/llama-3.1-70b-instruct': { label: 'Llama 3.1 70B', provider: 'Meta' },
      'meta-llama/llama-3.1-8b-instruct': { label: 'Llama 3.1 8B', provider: 'Meta' },
      'google/gemini-pro-1.5': { label: 'Gemini Pro 1.5', provider: 'Google' },
      'google/gemini-flash-1.5': { label: 'Gemini Flash 1.5', provider: 'Google' },
      'mistralai/mistral-large': { label: 'Mistral Large', provider: 'Mistral' },
      'mistralai/mistral-small': { label: 'Mistral Small', provider: 'Mistral' },
      'perplexity/llama-3.1-sonar-large-128k-online': { label: 'Perplexity Sonar Large', provider: 'Perplexity' },
      'cohere/command-r-plus': { label: 'Command R+', provider: 'Cohere' }
    };

    const leaderboardData = modelStats.length > 0 ? modelStats.map((stat, index) => {
      if (!stat.selectedModel) return null;
      
      const responseTimeData = modelResponseTimes.find(rt => rt.modelId === stat.selectedModel);
      const modelInfo = modelLabels[stat.selectedModel] || { label: stat.selectedModel, provider: 'Unknown' };
      ...
      const winRate = totalComparisons > 0 
        ? Math.round((stat._count.selectedModel / totalComparisons) * 100)
        : 0;

      const recentActivity = recentComparisons.filter(c => c.selectedModel === stat.selectedModel).length;
      ...
      const categoryMultiplier = getCategoryMultiplier(stat.selectedModel, category);
      const adjustedScore = Math.round(combinedScore * categoryMultiplier);
    ...
    Object.entries(externalBenchmarks).map(([modelKey, benchmarkData], index) => {
      const fullModelId = Object.keys(MODEL_MAPPING).find(key => MODEL_MAPPING[key] === modelKey) || modelKey;
      const modelInfo = modelLabels[fullModelId] || { label: modelKey, provider: 'Unknown' };
      
      const categoryMultiplier = getCategoryMultiplier(fullModelId, category);
    ...
    leaderboardData.sort((a, b) => {
      switch (category) {
        ...
        case 'copilot':
          return b.avgResponseTime > 0 ? a.avgResponseTime - b.avgResponseTime : b.score - a.score; // Speed priority
        default:
          return b.score - a.score;
      }
    });
    ...
    const globalStats = {
      ...
      avgResponseTime: modelResponseTimes.length > 0 
        ? Math.round(modelResponseTimes.reduce((sum, rt) => sum + rt.avgResponseTime, 0) / modelResponseTimes.length)
        : Math.round(leaderboardData.reduce((sum, model) => sum + model.avgResponseTime, 0) / (leaderboardData.length || 1)),
