...
function createAgentSystemPrompt(agentPersonality: string, topic: string, roomName: string): string {
...
Generate a natural, engaging comment about this topic that fits your personality:`
  ...
  if (agentPersonality.includes('Optimist') || agentPersonality.includes('Tech')) {
    return basePrompt + `\n\nAs a ${agentPersonality}, you should be enthusiastic, forward-thinking, and focus on positive possibilities and innovations.`
  } else if (agentPersonality.includes('Analyst') || agentPersonality.includes('Cautious')) {
...
async function callOpenRouter(prompt: string, model: string): Promise<{ response: string; responseTime: number }> {
  ...
  try {
    ...
    const responseTime = Date.now() - startTime
    return {
      response: 'Sorry, I encountered an error generating this response. Please try again.',
      responseTime
    }
  }
}
...
export async function POST(request: NextRequest) {
  try {
    ...
    if (body.messageType === 'agent_monologue') {
      ...
      console.log('🤖 Generating agent monologue for:', agentPersonality, 'on topic:', topic)
      
      if (!topic || !agentPersonality) {
        return NextResponse.json({ error: 'Topic and agent personality required for monologue' }, { status: 400 })
      }

      const systemPrompt = createAgentSystemPrompt(agentPersonality, topic, roomName)
      const contextPrompt = conversationHistory && conversationHistory.length > 0 
        ? `\n\nPrevious conversation context:\n${conversationHistory.map((msg: any) => `${msg.speaker}: ${msg.message}`).join('\n')}`
        : ''

      const fullPrompt = systemPrompt + contextPrompt

      console.log('📝 Generated prompt for agent:', fullPrompt.substring(0, 200) + '...')

      const result = await callOpenRouter(fullPrompt, 'openai/gpt-4o-mini')
      
      console.log('✅ Agent response generated, length:', result.response.length)
    ...
    if (!prompt || !models || models.length !== 2) {
      return NextResponse.json({ error: 'Invalid request. Prompt and exactly 2 models required.' }, { status: 400 })
    }
    ...
    if (validModels.length !== 2) {
      return NextResponse.json({ error: 'Invalid models selected' }, { status: 400 })
    }
